...
- **Коммить по фиче, а не накапливать.** Каждую завершённую фичу — отдельным
  коммитом, как только она проходит typecheck. Не давать рабочему дереву
  разрастаться на тысячи строк из нескольких фич сразу: такие «снежные комы»
  перемешиваются в общих файлах и их уже нельзя чисто разбить на осмысленные
  коммиты (только один смешанный снапшот).
- Не коммить артефакты сборки (`dist/`, `*.tsbuildinfo`) — они в `.gitignore`.
