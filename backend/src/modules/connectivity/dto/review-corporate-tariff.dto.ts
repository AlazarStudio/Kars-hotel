...
export class ReviewCorporateTariffDto {
  ...
  @ApiPropertyOptional({
    description:
      'Расхождения построчно: {categoryName, expected, actual} — обе суммы в ' +
      'КОПЕЙКАХ, как всё в контракте. Разные единицы в одном объекте однажды ' +
      'покажут расхождение в сто раз, и это не заметят.',
    type: 'array',
    items: { type: 'object' },
  })
  @IsOptional()
